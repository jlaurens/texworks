...
It can be deactivated turning `OFF` `TWX_NO_COLOR`.
...
unset ( __TWX_<key_i>__ )
```
for each suitable `i`.
#]=======]
function ( twx_configure_file_set )
  while ( True )
    list( LENGTH ARGN _n )
    if ( _n LESS 1 )
      return ()
    endif ()
    list ( GET ARGN 0 _key )
    list ( REMOVE_AT ARGN 0 )
    list( LENGTH ARGN _n )
    if ( _n LESS 1 )
      message ( FATAL_ERROR "Missing value")
    endif ()
    list ( GET ARGN 0 _value )
    list ( REMOVE_AT ARGN 0 )
    set ( __TWX_${_key}__ "${_value}" PARENT_SCOPE )
    if ( TWX_CONFIG_VERBOSE )
      message ( STATUS "twx_configure_file_set: ${_key} => ${_value}" )
    endif ()
  endwhile ()
endfunction ()

function ( twx_configure_file_unset )
  while ( True )
    list( LENGTH ARGN _n )
    if ( _n LESS 1 )
      return ()
    ...
    list ( GET ARGN 0 _key )
    list ( REMOVE_AT ARGN 0 )
    unset ( __TWX_${_key}__ PARENT_SCOPE )
  endwhile ()
endfunction ()
