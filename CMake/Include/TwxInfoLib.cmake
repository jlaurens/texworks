...
# ANCHOR: twx_info_path
#[=======[
Get the standard locations for static and git info
```
twx_info_path ( <variable> <name> [STAMPED])
```
...
for `stamped`.
#]=======]
function ( twx_info_path ans_ name_ )
  if ( EXISTS "${name_}" )
    set (
      ${ans_}
      "${name_}"
...
Usage:
```
twx_info_write_end ( <info_name> )
```
Do the writing...
#]=======]
function ( twx_info_write_end name_ )
  twx_info_path ( path_ "${name_}" )
  set (
    contents_ "\
;READ ONLY
...
Usage:
```
twx_info_read ( [ <info_name> | <file_path> ...] [QUIET] [ONLY_CONFIGURE] )
```
Argument: one of
...
# ANCHOR: twx_info_return_if_exists
#[=======[
Usage:
```
twx_info_return_if_exists ( name )
```
`name` is the argument of `twx_info_path`.
#]=======]
macro (twx_info_return_if_exists _name )
  twx_info_path ( TWX_path ${_name} )
  if ( EXISTS "${TWX_path}" )
    unset ( TWX_path )
    return ()
  endif ()
