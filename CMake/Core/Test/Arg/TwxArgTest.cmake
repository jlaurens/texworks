...
if ( /TWX/TEST/UNIT.RUN )
  ...
  function ( TwxArgTest_test1 op right status )
    twx_test_simple_check ( "${ARGC} ${op} ${right}" )
    twx_arg_assert_count ( ${ARGC} ${op} ${right} )
    cmake_language ( CALL "twx_test_simple_${status}" )
  endfunction ()
...
if ( /TWX/TEST/UNIT.RUN )
  ...
  function ( TwxArgTest_pass_option_2 CHI FOO )
    twx_test_simple_check ( "CHI => ``${CHI}'', FOO => ``${FOO}''" )
    set ( twx.R_CHI ${CHI} )
    set ( twx.R_FOO ${FOO} )
...
if ( /TWX/TEST/UNIT.RUN )
  block ()
  twx_test_simple_check ( "actual => ``EXPECTED''" )
  set ( actual EXPECTED )
  twx_arg_expect_keyword ( actual "EXPECTED" )
  twx_test_simple_pass ()
  twx_test_simple_check ( "actual => ``UNEXPECTED''" )
...
if ( /TWX/TEST/UNIT.RUN )
  block ()
  twx_test_simple_check ( "twx.R_FOO_B. => ``FOO'', twx.R_FOO_BAR. => ``FOO_BAR''" )
  set ( twx.R_FOO_B. FOO )
  ...
  twx_test_simple_check ( "twx.R_FOO_B. => ``FOOX'', twx.R_FOO_BAR. => ``FOO_BAR''" )
  set ( twx.R_FOO_B. FOOX )
  set ( twx.R_FOO_BAR. FOO_BAR )
  twx_arg_assert_keyword ( twx.R_FOO_B. twx.R_FOO_BAR. )
  twx_test_simple_fail ()

  twx_test_simple_check ( "twx.R_FOO_B. => ``FOO'', twx.R_FOO_BAR. => ``FOO_BARX''" )
  set ( twx.R_FOO_B. FOO )
  ...
  twx_test_simple_check ( "MY_FOO_B. => ``FOO''" )
  set ( MY_FOO_B. FOO )
  twx_arg_assert_keyword ( FOO_B. PREFIX MY )
  twx_test_simple_pass ()

  twx_test_simple_check ( "MY_FOO_B. => ``FOO'', MY_FOO_BAR. => ``FOO_BAR''" )
  set ( MY_FOO_B. FOO )
  ...
  twx_test_simple_check ( "MY_FOO_B. => ``FOO'', MY_FOO_BAR. => ``FOO_BARX''" )
  set ( MY_FOO_B. FOO )
...
if ( /TWX/TEST/UNIT.RUN )
  ...
  function ( TwxArgLib_assert_parsed_1 )
    ...
    twx_test_simple_check ( "${pretty_}" )
    cmake_parse_arguments ( twx.R "" "KEY;STATUS" "" ${ARGV} )
    twx_arg_assert_parsed ()
    cmake_language ( CALL "twx_test_simple_${twx.R_STATUS}" )
  endfunction()
