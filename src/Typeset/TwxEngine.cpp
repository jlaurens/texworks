...
namespace Key {
	auto const Engine = QStringLiteral("Engine");
	auto const name = QStringLiteral("name");
	auto const program = QStringLiteral("program");
	auto const arguments = QStringLiteral("arguments");
	auto const showPdf = QStringLiteral("showPdf");
}
...
QProcess* Engine::run(const QFileInfo& input, QObject* parent /* = nullptr*/) const
{
	...
	QStringList envPaths = env.value(QStringLiteral("PATH")).split(Core::pathListSeparator);
	envPaths.append(QFileInfo(exeFilePath).dir().absolutePath());
	env.insert(QStringLiteral("PATH"), envPaths.join(Core::pathListSeparator));
#endif
	...
	if (!checkedForSynctex) {
		...
		QString pdftex = PathManager::programPath(QStringLiteral("pdftex_TwxTypeset"));
#else
...
const QList<Engine>& Engine::factoryEngineList()
{
	static QList<Engine> list;
	if (list.empty()) {
		list
