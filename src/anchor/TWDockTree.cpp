...
const QString treeWidget_m  = QStringLiteral("treeWidget_m");
const QString toolbar_m     = QStringLiteral("toolbar_m");
const QString list_add_m    = QStringLiteral("list_add_m");
const QString list_remove_m = QStringLiteral("list_remove_m");
const QString Tags_m        = QStringLiteral("Tags_m");
const QString Bookmarks_m   = QStringLiteral("Bookmarks_m");
const QString Outlines_m    = QStringLiteral("Outlines_m");
...
QTextEdit *DockTree::textEdit()
{
    if (! textEdit_m) {
        auto *p = parent();
        if (p) {
            textEdit_m = p->findChild<QTextEdit *>(ObjectName::textEdit_m);
        }
        if (! textEdit_m) {
            auto *w = windowHandle();
            if (w) {
                textEdit_m = w->findChild<QTextEdit *>(ObjectName::textEdit_m);
            }
        }
        if (! textEdit_m) {
            qDebug() << "No editor";
